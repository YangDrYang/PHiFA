function seg = makeSolarPanelLeft(longSide, shortSide, thickness, resolution)
...
diameter = 2.8;
seg.segmentReferenceCube = [0 0.5*diameter+longSide 0.5*thickness; -shortSide 0.5*diameter 0.5*thickness].';
x = zeros(resolution);
y = zeros(resulution);
ori = [-shortSide,0.5*diameter];
x(end) = 0;
y(edn) = 0.5*diameter+longSide;
