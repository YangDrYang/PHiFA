function [pert, prop, vars] = reformatSimulationData_radau(is, sim_out)
...
%         break;
%     end
    time(i) = sim_out(i+1).t(end);
    eph(1,i) = time(i);
    
    vars(1,i) = time(i);
    vars(2,i) = sim_out(i+1).lt;
    
    eph(2:14,i) = sim_out(i+1).Y;
    if is.a_lase
        laser_imp(1:3,i) = sim_out(i+1).laser.imp;
        laser_imp(4,i) = norm(sim_out(i+1).laser.imp);
        laser_for(1:3,i) = sim_out(i+1).laser.for;
        laser_for(4,i) = norm(sim_out(i+1).laser.for);
    else
        laser_imp(1:4,i) = [0;0;0;0];
        laser_for(1:4,i) = [0;0;0;0];
    end
    if is.a_drag
        drag(1:3,i) = sim_out(i+1).drag.for;
        drag(4,i) = norm(sim_out(i+1).drag.for);
    else
        drag(1:4,i) = [0;0;0;0];
    end
    if is.a_srad
        srad(1:3,i) = sim_out(i+1).srad.for;
        srad(4,i) = norm(sim_out(i+1).srad.for);
    else
        srad(1:4,i) = [0;0;0;0];
    end
    if is.a_grav
        grav(1:3,i) = sim_out(i+1).grav.for;
        grav(4,i) = norm(sim_out(i+1).grav.for);
    else
        grav(1:4,i) = [0;0;0;0];
    end
    
    if is.g_lase
        laser_mom(1:3,i) = sim_out(i+1).laser.mom;
        laser_mom(4,i) = norm(sim_out(i+1).laser.mom);
        laser_tor(1:3,i) = sim_out(i+1).laser.tor;
        laser_tor(4,i) = norm(sim_out(i+1).laser.tor);
    else
        laser_mom(1:4,i) = [0;0;0;0];
        laser_tor(1:4,i) = [0;0;0;0];
    end
    if is.g_drag
        dragt(1:3,i) = sim_out(i+1).drag.tor;
        dragt(4,i) = norm(sim_out(i+1).drag.tor);
    else
        dragt(1:4,i) = [0;0;0;0];
    end
    if is.g_srad
        sradt(1:3,i) = sim_out(i+1).srad.tor;
        sradt(4,i) = norm(sim_out(i+1).srad.tor);
    else
        sradt(1:4,i) = [0;0;0;0];
    end
    if is.g_grav
        gravt(1:3,i) = sim_out(i+1).grav.tor;
        gravt(4,i) = norm(sim_out(i+1).grav.tor);
    else
        gravt(1:4,i) = [0;0;0;0];
    end
end
